...
from app.domain.runner_protocol import ConfigBundle, EventBatch, Heartbeat, HeartbeatResponse, Command
...
async def tenant_heartbeat(
    ...
    await db.commit()
    return HeartbeatResponse(server_time=utcnow(), desired_config_version=(await control.get_config(db, user_id)).version, commands=[]).model_dump(mode="json")
