...
class DexPaprikaClient:
    ...
    async def token_pools(self, network: str, token_address: str) -> list[dict]:
        """Best-effort: list pools containing a token.

        DexPaprika's public docs don't advertise a token→pools endpoint
        explicitly, but the token details response contains a ``pools`` array
        on the live API. If it's missing, callers fall back to discovery-time
        pool addresses captured in the provider.
        """
